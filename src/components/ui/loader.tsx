...
export default function Loader({ tag = 'div', size = 'medium', variant = 'blink', showOnlyThreeDots, className }: LoaderTypes) {
    ...
    return (
        <Component className={cn('flex items-center gap-2', variant === 'moveUp' && handleLoaderPosition(size), className)}>
            <span className={cn('rounded-full bg-brand dark:bg-white', handleVariantClasses(variant, size), sizes[size])} />
            <span className={cn('animation-delay-200 rounded-full bg-brand dark:bg-white', handleVariantClasses(variant, size), sizes[size])} />
            <span className={cn('animation-delay-500 rounded-full bg-brand dark:bg-white', handleVariantClasses(variant, size), sizes[size])} />
            {variant === 'moveUp' && !showOnlyThreeDots ? <span className={cn('animation-delay-700 rounded-full bg-brand dark:bg-white', handleVariantClasses(variant, size), sizes[size])} /> : null}
        </Component>
    );
}
